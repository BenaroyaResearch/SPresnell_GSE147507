...
Volcano(fit2, "postInfection", "GSE147507 Lung Adeno SARS-CoV-2 infected vs mock", labelSource = "genes", write=T)
```
...
intMM <- model.matrix(~Treatment, data=testDesign)
colnames(intMM) <- c("(Intercept)", "postRSVInfection")
...
Volcano(fit2, "postRSVInfection", "GSE147507 Lung AC cells RSV infected vs mock", labelSource = "genes", write=T)
```
...
PlotGeneExpression <- function(counts, design, genes, plotTitle, treatmentLevels=c("mock", "CoV2"), fileType="png") {
  #Add gene expression (lt2 +1) counts to design
  selected_gene_exprs <- log2(counts[genes,]+1)
  ...
  design_gathered$Treatment <- factor(design_gathered$Treatment, treatmentLevels)
...
counts_norm_SARS <- edgeR::cpm(curSARSDGE, normalized.lib.sizes=TRUE)
...
counts_norm_LungAC <- edgeR::cpm(LungACDGE, normalized.lib.sizes=TRUE)
PlotGeneExpression(counts_norm_LungAC, filteredLungACDesign, interesting_genes, "GSE147507 Interesting Genes From Lung AC with SARS-CoV-2")

counts_norm_LungRSV <- edgeR::cpm(LungRSVDGE, normalized.lib.sizes=TRUE)
PlotGeneExpression(counts_norm_LungRSV, filteredLungRSVDesign, interesting_genes, "GSE147507 Interesting Genes From Lung AC with RSV", treatmentLevels=c("mock", "RSV"))
